...
twitter_sample <- subset_text(twitter, 0.3)
blogs_sample <- subset_text(blogs, 0.3)
news_sample <- subset_text(news, 0.3)
...
ngram_lookup <- function(text, ngram_df, coloc = FALSE) {
    model_df <- ngram_df %>%
        dplyr::mutate(features = as.character(features)) %>%
        {
            if(coloc == FALSE) dplyr::filter(., grepl(paste0("^", text, "_"), features)) else { 
                dplyr::filter(., grepl(text, features)) 
                }
        } %>%
...
coloc_predict <- function(input_text, sw = FALSE, num = 5) {
    #Tokenize input text
    input_tokens <- clean_text(input_text, stem = FALSE, stopwords = FALSE)[[1]]
    ...
    if(length(input_tokens) > 2) {
        text <- input_tokens[(length(input_tokens) - 2):length(input_tokens)]
        words <- ngram_lookup(paste(text, collapse = "|"), top4_1, coloc = TRUE)
        if(length(words) < 6) {
            text <- text[-1]
            words <- unique(c(words, ngram_lookup(paste(text, collapse = "_"), top3_1, coloc = TRUE)))
            if(length(words) < 6) {
                text <- text[-1]
                words <- unique(c(words, ngram_lookup(paste(text, collapse = "_"), top2_1)))
                if(length(words) < 6) {
